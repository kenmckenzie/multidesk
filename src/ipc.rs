...
fn seed_multidesk_default_permanent_password() -> bool {
    ...
    match set_permanent_password_with_ack(MULTIDESK_DEFAULT_PERMANENT_PASSWORD.to_owned()) {
        ...
        Ok(false) => {
            log::warn!("MultiDesk default unattended access password was rejected by daemon");
            false
        }
        Err(err) => {
            log::warn!("Failed to persist MultiDesk default unattended access password: {err}");
            false
        }
    }
}
...
pub fn is_permanent_password_set() -> bool {
    match get_config("permanent-password-set") {
        Ok(Some(v)) => {
            ...
            if v == "N" {
                return seed_multidesk_default_permanent_password();
            }
        }
        Ok(None) => {
